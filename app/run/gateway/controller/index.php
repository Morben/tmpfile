...
class index
{
    ...
    public function checkpoint()
    {
        ...
        //trade_no -> 自动生成
        $data['trade_no'] = mt_rand(10000, 99999) . date('Ymd') . substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
        //amount
    ...
    private function automatic($user, $type_content, $data)
    {
      if ($data['amount'] > 15000) functions::str_json($type_content, -1, '支付金额不能大于15000元');
        ...
        //已经得到wechat参数
        if (empty($find_wechat)) functions::str_json($type_content, -1, 'automatic->无可用通道,请联系客服');
        ...
        if ($create_order > 0) {
            $mark = $find_wechat['id'] . '|' . $create_order;
            \xh\library\gateway::getQrCode($find_wechat['user_id'], $mark, $data['amount'], $find_wechat['key_id'], 'wechat');

            if ($type_content == 'json') {

                $qrcode = functions::syncRequest('wechat_' . intval($create_order), 50000);
    ...
    private function alipay($user,$type_content,$data){
        ...
        if ($data['robin'] == 2) {
            ...
            if ($robin == 1){
                //随机算法
                $find_alipay = $this->mysql->query("client_alipay_automatic_account","status=1 and user_id={$user['id']} and training=1 and receiving=1 ");
            ...
            //实时收款算法，按少到多排序 v1.0
            if ($robin == 2){
                $find_alipay = $this->mysql->query("client_alipay_automatic_account","status=1 and user_id={$user['id']} and training=1 and receiving=1 ",null,"today_money","asc");
                $find_alipay = $find_alipay[0];
            }
            
            //顺序模式算法 v1.0
            if ($robin == 3){
                $find_alipay = $this->mysql->query("client_alipay_automatic_account","status=1 and user_id={$user['id']} and training=1 and receiving=1 ",null,"id","asc");
                $find_alipay = $find_alipay[0];
            }
            ...
            if ($find_alipay['status'] != 1 || $user['id'] != $find_alipay['user_id'] || $find_alipay['receiving'] != 1 ) functions::str_json($type_content, -1, 'automatic->初始化失败,当前支付通道不可使用');
        }
        //已经得到alipay参数
        //生成订单
        ...
        functions::str_json($type_content, -1, 'BankAUTOMATIC->22点到8点不再支持测试需要测试的客户请白天测试-订单创建失败');
    }
   ...
   //服务版
    private function service($user,$type_content,$data){
        ...
        if ($data['robin'] == 2) {
            ...
            if ($robin == 1){
                //随机算法
                $find_service = $this->mysql->query("service_account","status=4 and training=1 and receiving=1  and types={$data['type']} {$where_system}");
            ...
            //实时收款算法，按少到多排序 v1.0
            if ($robin == 2){
                $find_service = $this->mysql->query("service_account","status=4 and training=1 and receiving=1 and types={$data['type']} {$where_system}",null,"today_money","asc");
                $find_service = $find_service[0];
            }
            //顺序模式算法 v1.0
            if ($robin == 3){
                $find_service = $this->mysql->query("service_account","status=4 and training=1 and receiving=1  and types={$data['type']} {$where_system}",null,"id","asc");
                $find_service = $find_service[0];
            }
        }else{
            //指定微信号 单条进入
            ...
            if ($find_service['status'] != 4 || $find_service['receiving'] != 1 ) functions::str_json($type_content, -1, 'service->请稍后,支付通道抢修中..');
        }
