...
class service{
    ...
    //权限ID：27
     public function add(){
       $this->powerLogin(27);
        ...
        $card = request::filter('get.cardid', '', 'htmlspecialchars');
       	$name = request::filter('get.account', '', 'htmlspecialchars');
        //计算keyid
   ...
   //订单管理
        ...
        if ($order['pay_time'] == 0){
            ...
            $pay_time = $order['pay_time'];
               ...
               $group = $this->mysql->query('client_group',"id={$user['group_id']}")[0];
                //解析数据
               $authority = json_decode($group['authority'],true)[$module_name];                       //判断用户组是否存在
                if (is_array($group) || $group['authority'] != -1 || $authority['open'] == 1) {
                            //手续费扣掉后的金额
                            $fees = $order['amount'] * $authority['cost'];
                  			$usermoney = $order['amount'] - $fees;
                            $tixian = $user['money'] + $usermoney;
      		  			if (intval($order['reached']) === 0){
                            //给用户加钱
                            $deductionStatus = $this->mysql->update("client_user", [
                                'money' => $tixian
                            ], "id={$user['id']}");
                            //直接强制修改reached
                            $this->mysql->update("client_alipay_automatic_orders", ['reached'=>1],"id={$order['id']}");
                        }
    ...
    public function voiceBroadcast(){
        $this->powerLogin(26);
        $order = $this->mysql->query("service_order","voice=0")[0];
        if (is_array($order)) {
            $this->mysql->update("service_order", ['voice'=>1],"id={$order['id']}");
            functions::json(200, '有新订单已支付');
        }else{
            functions::json(-3, '暂时无播报');
        }
    }
