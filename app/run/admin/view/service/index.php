  ...
  <div class="row">
    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        ...
        <!-- <button type="button" onclick="addWechat();" class="btn btn-info btn-xs"><i class="fa fa-plus"></i> 新增普通银行</button>-->
		 <!--<button type="button" onclick="addAlipay();" class="btn btn-info btn-xs"><i class="fa fa-plus"></i> 新增支付宝</button>-->

		 <button type="button" onclick="addAlipayNew();" class="btn btn-info btn-xs"><i class="fa fa-plus"></i> 新增支付宝红包</button>

		 
		 <button type="button" onclick="robinTest(1);" class="btn btn-info btn-xs"><i class="fa fa-xing-square"></i> 通道测试(支付宝红包)</button>
        ...
        <div class="panel-body table-responsive">
          <table class="table table-hover">
            <thead>
              <tr>
                <td>设备信息  [ <a href="<?php echo url::s('admin/service/index','sorting=type&code=go');?>">全部</a> / <a href="<?php echo url::s('admin/service/index','sorting=type&code=1');?>">支付宝</a> / <a href="<?php echo url::s('admin/service/index','sorting=type&code=2');?>">网商</a> ]</td>
            ...
            <?php  foreach ($result['result'] as $ru){  ?>
              <tr>
                <td>
                    <p><b>账号</b>: <?php echo $ru['name'] == '0' ? '<span style="color:red">Unused</span>' : '<span style="color:red">'.$ru['name'].'</span>';?>  [ 今日总额: <span style="color: green;"><?php echo $ru['today_money'];?></span> / 笔数: <span style="color: red;"><?php echo $ru['today_pens'];?></span> ] ( <a href="<?php echo url::s('admin/service/order',"sorting=service&code={$ru['id']}");?>">交易订单</a> )</p>
					<p><b>设备KEY: </b> <?php echo $ru['key_id'];?> <?php if ($ru['status']!=6 && $ru['status']!=4)?> </p>

                </td>
                  <td>
                   <p>状态:已登录</p>
                    <p>添加时间: <?php echo $ru['login_time'] == 0 ? '从未登录' : date("Y/m/d H:i:s",$ru['login_time']);?></p>
                </td>
                ...
                <td>
                        ...
                        ?></p>
                         ...
                         <td>
                        <p><b>账号类型:</b> <?php if ($ru['types'] == 1) echo '<span style="color:green;">支付宝</span>'; if ($ru['types'] == 2) echo '<span style="color:red;">网商</span>'; ?> [ <?php if ($ru['lord'] == 0){?><a href="#" onclick="setLord('<?php echo $ru['id'];?>');" style="color: green;font-size:8px;">设置为系统主用</a><?php }else {?><a href="#" onclick="stopLord('<?php echo $ru['id'];?>');" style="color: red;font-size:8px;">取消系统主用</a><?php }?> ] [ <a href="#" onclick="gatewayTest('<?php echo $ru['key_id'];?>','<?php echo $ru['types'];?>')">单通道测试</a> ]</p>
                        <p><b>全部收入:</b> <?php 
    ...
    <!-- End Panel -->  
            ...
            function addWechat(){
						...
						shade: 0.8,
						area: ['560px', '480px'],
						content: '<?php echo url::s('admin/service/viewAdd');?>' //iframe的url
			}); 
            ...
            function addAlipayNew(){
                swal({   title: "请输入支付宝配置信息",
                        ...
                        +"支付宝PID<input type='text' id='cardid'>",
                    ...
                    function(inputValue){
                        ...
                        $.get("<?php echo url::s('admin/service/add',"account=");?>" +$('#account').val() +  '&cardid='+ $('#cardid').val(), function(result){
                            if(result.code == '200'){
