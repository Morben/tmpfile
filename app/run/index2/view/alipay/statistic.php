                                ...
                                <th>
                                    <div class="input-field col s6" style="font-weight:normal;">
                                        ...
                                        <label>选择通道来查看<?php if ($_SESSION['ALIPAY']['ORDER']['WHERE'] == '') { ?>(<a
                                                    ...
                                                    href="<?php echo url::s('index2/alipay/statisticOrder', "sorting=alipay&locking=closed"); ?>">取消锁定</a>)<?php } ?>
                                        </label>
                                    </div>
                                </th>
                                <th>
                                    支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status') { ?>(<?php if ($sorting['code'] == 1) echo '获取订单中';
                                        ...
                                        if ($sorting['code'] == 4) echo '已支付'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index2/alipay/statisticOrder', "sorting=status&code=" . ($sorting['code'] + 1)); ?>'><i
                                ...
                                <th>
                                    异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback') { ?>(<?php if ($_GET['code'] == 0) echo '未回调';
                                        if ($_GET['code'] == 1) echo '已回调'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index2/alipay/statisticOrder', "sorting=callback&code=" . ($sorting['code'] + 1)); ?>'><i
                                                class="mdi-image-healing"></i></a></th>
                                <th>回调发送次数</th>
                                <th>回调来源</th>
                                <th>回调信息</th>
...
<script type="text/javascript">
    ...
    function trade_no(obj) {
        ...
        if (start_time || end_time) {
            location.href = "<?php echo url::s('index2/alipay/statisticOrder', "sorting=trade_no&");?>"+'&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index2/alipay/statisticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
        }
    }

    function alipay() {
        var alipay = $('#alipay').val();
        location.href = "<?php echo url::s('index2/alipay/statisticOrder', "sorting=alipay&code=");?>" + alipay;
    }
    ...
    function exportData () {
        ...
        if (code) {
            location.href = "<?php echo url::s('index2/alipay/export', "sorting=alipay&code=");?>" + code+ '&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index2/alipay/export', "sorting=alipay&start_time=");?>" + start_time + '&end_time=' + end_time;
        }
    }
