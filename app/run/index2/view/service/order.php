...
?>
    ...
    font-size: 14px;" type="submit" value="查询"></th>
                  ...
                  <div class="input-field col s6" style="font-weight:normal;">
                    ...
                    <label> [ <a href="<?php echo url::s('index2/service/order',"sorting=gateway&code=all");?>">全部</a> / <a href="<?php echo url::s('index2/service/order',"sorting=gateway&code=wechat");?>">支付宝红包</a> / <a href="<?php echo url::s('index2/service/order',"sorting=gateway&code=alipay");?>">支付宝</a> ] <?php if ($_SESSION['SERVICE']['ORDER']['WHERE'] == ''){?>(<a href="#" onclick="service();">开始查询</a>)<?php }else{?>(<a href="<?php echo url::s('index2/service/order',"sorting=service&code=closed");?>">取消</a>)<?php }?></label>
                  </div>
                
                     	</th>
                        <th>支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status'){?>(<?php if ($sorting['code'] == 1) echo '获取订单中';if ($sorting['code'] == 2) echo '未支付';if ($sorting['code'] == 3) echo '订单超时';if ($sorting['code'] == 4) echo '已支付';?>)<?php }?><a href='<?php echo url::s('index2/service/order',"sorting=status&code=".($sorting['code']+1));?>'><i class="mdi-image-healing"></i></a></th>
                        <th>异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback'){?>(<?php if ($_GET['code'] == 0) echo '未回调';if ($_GET['code'] == 1) echo '已回调';?>)<?php }?><a href='<?php echo url::s('index2/service/order',"sorting=callback&code=".($sorting['code']+1));?>'><i class="mdi-image-healing"></i></a></th>
                        <th>回调信息 [ 回调测试: <a href="#" onclick="robinTest(1);">支付宝红包</a> / <a href="#" onclick="robinTest(2);">支付宝</a> ]</th>
                          <th style="
    ...
    font-size: 14px;" onclick="exportData()">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        <td>支付类型：<?php if ($ru['types'] == 1) echo '<b style="color:red;">支付宝红包</b>';?><?php if ($ru['types'] == 2) echo '<b style="color:red;">支付宝</b>';?> / 订单ID：<?php echo $ru['id'];?> ( <a target="_blank" href="<?php echo url::s('gateway/pay/service',"id={$ru['id']}");?>">支付链接</a> )
      ...
      function trade_no(obj){
          location.href = "<?php echo url::s('index2/service/order',"sorting=trade_no&code=");?>" + $(obj).val();
          }
      ...
      function service(){
          ...
          location.href = "<?php echo url::s('index2/service/order',"sorting=service&code=");?>" + service;
          }
      function exportData () {
          var code = GetQueryString('code');
          var start_time = GetQueryString('start_time');
          var end_time = GetQueryString('end_time');
          location.href = "<?php echo url::s('index2/service/export', "sorting=trade_no&start_time=");?>" + start_time + '&end_time=' + end_time + '&code=' + code;
      }
