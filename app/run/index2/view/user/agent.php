...
<!--                      </select>-->
<!--                      <label>选择类型查看--><?php //if ($_SESSION['WECHAT']['ORDER']['WHERE'] == ''){?><!--(<a href="#" onclick="wechat();">开始查询</a>)--><?php //}else{?><!--(<a href="--><?php //echo url::s('index2/wechat/automaticOrder',"sorting=wechat&locking=closed");?><!--">取消锁定</a>)--><?php //}?>
<!--                      </label>-->
<!--                  </div>-->
                <div class="col s12 m12 l12">
    ...
    font-size: 14px;" onclick="editRate(<?php echo $ru['id']?>,<?php echo $ru['rate']['wechat_auto'] ? $ru['rate']['wechat_auto'] : 0?>,<?php echo $ru['rate']['alipay_auto'] ? $ru['rate']['alipay_auto'] : 0?>,<?php echo $ru['rate']['service_auto'] ? $ru['rate']['service_auto'] : 0?>)">设置费率</a>
          ...
          function editRate(id,wechat_auto,alipay_auto,service_auto){
              swal({   title: "设置费率",
                  ...
                  function(inputValue){
                      ...
                      $.get("<?php echo url::s('index2/member/rate',"id=");?>" + id + "&wechat_auto=" + $('#wechat_auto').val() + "&type=edit&alipay_auto=" + $('#alipay_auto').val()+ "&type=edit&service_auto=" + $('#service_auto').val(), function(result){
                          if(result.code == '200'){
          ...
          function addAgent(uid){
              swal({   title: "添加代理",
                  ...
                  function(inputValue){
                      ...
                      $.get("<?php echo url::s('index2/member/addAgent',"uid=");?>" + uid + "&userId="+$('#userId').val(), function(result){
                          if(result.code == '200'){
                              swal("成功提醒", result.msg, "success");
                              setTimeout(function(){location.href = '';},1000);
                          }else{
      ...
      function note(obj){
          location.href = "<?php echo url::s('index2/member/agent',"sorting=note&name=");?>" + $(obj).val();
          }
