...
?>
    ...
    <!-- START CONTENT -->
                ...
                <div class="col s12 m12 l12">
                  <table class="striped"  style="font-size: 14px;">
                    <thead>
                      <tr>
                     	<th>类型</th>
                        <th><div class="input-field col s6"> <input onchange="note(this);" id="last_name" type="text" class="validate" value="<?php if ($sorting['name'] == 'note') echo $_GET['code'];?>"> <label for="last_name">直接输入网商订单号查询手动回调不会显示在记录</label></div><p><font color="red">手动回调的不会显示在收款记录</font></p></th>
                        <th>收款金额</th>
                        <th>设备版本</th>
                      </tr>
                    </thead>
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        <td>
                        <?php if ($ru['version_code'] == 'wechat_auto') echo '微信[公开版]';if ($ru['version_code'] == 'alipay_auto') echo '网商[公开版]';if ($ru['version_code'] == 'service_auto') echo '自动服务版';?>
                        </td>
      ...
      function note(obj){
          location.href = "<?php echo url::s('index2/member/record',"sorting=note&code=");?>" + $(obj).val();
          }
