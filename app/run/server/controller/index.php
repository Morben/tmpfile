...
//                    'receiving' => 2
    ...
    public function unlockFloorNum(){
        echo "unlock start!\n";
        $orders = $this->mysql->query('client_alipay_automatic_orders', "status!=4 and del_floor=0 order by id asc");
        $temp = [];

        //同一金额的订单，大于10分钟的需要解锁金额，但是有不大于10分钟的金额不能够解锁
        foreach($orders as $order){
            $now = time();
            if(($now - $order['creation_time']) >= 10*60){ //大余10分钟的需要解锁金额
                $temp[$order['amount']] = $order['amount'];
                $this->mysql->update('client_alipay_automatic_orders', ['del_floor'=>1], 'id='.$order['id']);
            }else{
                // 不大于10分钟的金额不能够解锁
                if(isset($temp[$order['amount']])){
                    unset($temp[$order['amount']]);
                }
            }
        }

        // 开始解锁金额
        $redis = $redis = redis::getInstance(functions::getRedisConfig());
        foreach ($temp as $val){
            $floorMoney = floor($val); // 把小数点去掉 如100.10转成100
            $floorNum = explode(".",$val)[1]/100;
            $redisKey = 'order:amount:lock:floor:'.$floorMoney;

            //解锁金额
            $res = $redis->srem($redisKey,$floorNum);
            $str = date('Y-m-d H:i:s')."-解锁浮动金额-$floorMoney".'-'.$floorNum.'=>解锁结果:'.$res."\n";
            echo $str;
            file_put_contents("jiaoben_unlock_floor.txt", $str, FILE_APPEND);

        }
        echo 'unlock done!';

    }
      public function unlockFloorNumm(){
        echo "unlock start!\n";
        $orders = $this->mysql->query('service_order', "status!=4 and del_floor=0 order by id asc");
        $temp = [];
        ...
        foreach($orders as $order){
            ...
            if(($now - $order['creation_time']) >= 10*60){ //大余10分钟的需要解锁金额
                $temp[$order['amount']] = $order['amount'];
                $this->mysql->update('service_order', ['del_floor'=>1], 'id='.$order['id']);
            }else{
                // 不大于10分钟的金额不能够解锁
                if(isset($temp[$order['amount']])){
                    unset($temp[$order['amount']]);
        ...
        foreach ($temp as $val){
            ...
            $str = date('Y-m-d H:i:s')."-解锁浮动金额-$floorMoney".'-'.$floorNum.'=>解锁结果:'.$res."\n";
            echo $str;
            file_put_contents("server_unlock_floor.txt", $str, FILE_APPEND);
